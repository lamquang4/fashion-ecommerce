...
function MainBannerTable({ mainbanners, isLoading, mutate }: Props) {
      ...
      <thead>
        <tr className="bg-[#E9EDF2] text-left">
          ...
          <th className="p-[1rem]   relative">
            <FilterDropDownMenu title="Loại" array={array1} paramName="type" />
          </th>
          <th className="p-[1rem]   relative">
            <FilterDropDownMenu
              title="Tình trạng"
        ...
        {isLoading ? (
          ...
          mainbanners.map((mainbanner) => (
            <tr key={mainbanner._id} className="hover:bg-[#f2f3f8]">
              ...
              <td className="p-[1rem]  ">
                {new Date(mainbanner.createdAt as string).toLocaleDateString(
                  "vi-VN",
                )}
              </td>
              <td className="p-[1rem]  ">
