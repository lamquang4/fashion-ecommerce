...
export async function POST(req: NextRequest) {
  try {
    ...
    for (
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (!allowedTypes.includes(file.type)) {
          return NextResponse.json(
            {
              msg: `Hình "${file.name}" ở biến thể thứ ${blockIndex + 1} không đúng định dạng PNG, JPG hoặc WEBP.`,
            },
            { status: 400 },
          );
        }
        if (file.size / 1024 > maxSizeKB) {
          return NextResponse.json(
            {
              msg: `Hình "${file.name}" ở biến thể thứ ${blockIndex + 1} vượt quá dung lượng ${maxSizeKB}KB.`,
            },
            { status: 400 },
          );
        }
    ...
    for (
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const result: any = await new Promise((resolve, reject) => {
          cloudinary.uploader
            .upload_stream(
              {
                folder: `aura-fashion/product/${newProduct._id}/${variant._id}`,
      ...
      await Inventory.findByIdAndUpdate(variant._id, { images: imagePaths });
    }
