...
function ProfileMenu({ profileMenuOpen }: Props) {
  ...
  if (status === "loading" || !profileMenuOpen) return null;

  return (
    <>
      {profileMenuOpen && (
        <div className="text-[0.9rem] absolute top-full right-0 z-20 bg-white shadow-md rounded-sm overflow-hidden min-w-[120px]">
          {session?.user ? (
            <>
